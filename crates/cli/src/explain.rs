...
pub fn coverage_analyze_meta() -> Value {
    json!({
        ...
        "field_definitions": {
            ...
            "runtime_coverage.findings[].stable_id": "Cross-surface JOIN key (fallow:fn:<hash>) from fallow_cov_protocol::function_identity_id, hashing file + name + start_line (NOT the line position). Stable across line moves; the same function shares this value across findings, hot paths, blast-radius, and importance entries. Omitted from the JSON entirely (not emitted as null) when the producing surface or an un-migrated cloud supplied no FunctionIdentity. New baselines key on this when present so suppressions survive line shifts.",
